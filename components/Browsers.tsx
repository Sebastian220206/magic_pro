...
export function Browsers() {
  ...
  const allFilesBase: BrowserFile[] = [
    ...
    return (
        <div className="w-[340px] h-full bg-studio-panel border-l border-black flex flex-col shrink-0 z-50 overflow-hidden shadow-[-30px_0_60px_rgba(0,0,0,0.6)] select-none text-studio-text-mid">
            <div className="pt-2 px-3 flex flex-col gap-2 shrink-0 border-b border-black pb-3 bg-studio-panel">
                ...
                <div className="flex bg-studio-sunken rounded-lg border border-studio-line p-0.5 h-8 shadow-inner">
                    ...
                    <button
                        ...
                        className={`flex-1 text-[10px] font-black uppercase transition-all rounded transition-all ${activeTab === 'all' ? 'text-accent-cyan bg-studio-control shadow-md border border-studio-line-strong' : 'text-studio-text-dim hover:text-studio-text-mid'}`}
                    >All Files</button>
                    <button
                        onClick={() => setActiveTab('sounds')}
                        className={`flex-1 text-[10px] font-black uppercase transition-all rounded transition-all ${activeTab === 'sounds' ? 'text-accent-cyan bg-studio-control shadow-md border border-studio-line-strong' : 'text-studio-text-dim hover:text-studio-text-mid'}`}
                    >Sounds</button>
                </div>
                ...
                <div className="mt-1 flex items-center gap-1 text-[9px] uppercase">
                    {['Computer','Home','Project'].map(loc => (
                        <button
                            ...
                            className={`flex-1 py-1 rounded-lg ${allFilesLocation === loc ? 'bg-accent-cyan/25 text-accent-cyan border border-accent-cyan/50' : 'bg-black/20 text-studio-text-mid hover:bg-white/10'} transition-colors`}
                        >{loc}</button>
                    ))}
                    <button
                        onClick={() => setViewMode(v => v === 'list' ? 'column' : 'list')}
                        className="px-2 py-1 rounded-lg bg-black/20 text-studio-text-mid hover:bg-white/10 transition-colors"
                ...
                <div className="relative mt-1">
                    ...
                    <input
                        ...
                        className="w-full bg-studio-sunken border border-studio-line rounded px-8 h-7 text-[11px] font-medium text-studio-text placeholder-studio-text-dim focus:outline-none focus:border-accent-cyan/30 shadow-inner group-hover:border-studio-line-strong transition-colors"
                        value={searchQuery}
                        onChange={(e) => setSearchQuery(e.target.value)}
                        onKeyDown={handleSearchKeyDown}
                        onClick={(e) => { e.stopPropagation(); setShowRecentSearch(true); }}
                    />
                    <button onClick={handleImportButtonClick} className="absolute right-2 top-1/2 -translate-y-1/2 p-1 hover:bg-white/5 rounded"><PlusCircle className="w-3 h-3 text-studio-text-dim" /></button>
                    ...
                    <div className="absolute right-8 top-1/2 -translate-y-1/2 p-1 cursor-pointer hover:bg-white/5 rounded" onClick={(e) => { e.stopPropagation(); setSearchQuery(''); }}><X className="w-3 h-3 text-studio-text-dim" /></div>
                </div>
            </div>
            ...
            <div className="flex-1 flex flex-col min-h-0 bg-studio-void">
                <div className="h-6 flex items-center bg-studio-raised border-b border-black text-[9px] font-black text-studio-text-dim uppercase px-3 gap-2 shrink-0 sticky top-0 z-10">
                    ...
                    <div className="w-12 text-center text-accent-cyan/40 opacity-0 group-hover:opacity-100"><Filter className="w-2.5 h-2.5" /></div>
                </div>
                    ...
                    {activeTab === 'project' ? (
                        projectFiles.length > 0 ? (
                            projectFiles.map((file, idx) => (
                                <div
                                    ...
                                    className={`h-8 flex items-center px-3 border-b border-black/10 group cursor-pointer hover:bg-accent-cyan/[0.08] transition-colors ${idx % 2 === 0 ? 'bg-white/[0.01]' : 'bg-transparent'}`}
                                >
                                    <div className="w-6 flex items-center justify-center">
                                        {file.type === 'audio' ? <FileAudio className="w-3.5 h-3.5 text-accent-cyan opacity-40 group-hover:opacity-100" /> : <Music className="w-3.5 h-3.5 text-green-500 opacity-40 group-hover:opacity-100" />}
                                    </div>
                                    <div className="flex-1 text-[11px] font-bold text-studio-text-dim group-hover:text-studio-text truncate pr-2">
                                        {file.name}
                                    </div>
                                    <div className="w-16 text-[10px] font-black text-studio-text-dim group-hover:text-studio-text-dim text-right tabular-nums">{file.size}</div>
                            ...
                            <div className="flex-1 flex flex-col items-center justify-center pt-20 text-studio-text scale-75 opacity-20"><FileText className="w-12 h-12 mb-2" /><span className="text-[10px] font-black uppercase tracking-widest">No Search Results</span></div>
                        )
                    ) : activeTab === 'sounds' ? (
                        <SoundLibraryBrowser
                        ...
                        filteredAllFiles.map((item, idx) => (
                            <div
                                ...
                                className={`h-11 flex items-center px-3 border-b border-black/10 group cursor-pointer transition-colors hover:bg-white/[0.05] border-l-2 border-transparent hover:border-accent-cyan/40`}
                            >
                                <div className="w-10 flex items-center justify-center">
                                    {item.type === 'drive' ? <HardDrive className="w-5 h-5 text-accent-cyan drop-shadow-lg" /> : <Folder className="w-5 h-5 text-studio-text-dim group-hover:text-accent-cyan shadow-sm" />}
                                </div>
                                <div className="flex-1 text-[12px] font-black text-studio-text-dim group-hover:text-studio-text truncate">
                                    {item.name}
                                </div>
                                <div className="w-16 text-[10px] font-black text-studio-text-dim text-right tabular-nums mr-2">{item.size || '-'}</div>
                                <ChevronRight className="w-3.5 h-3.5 text-studio-text opacity-0 group-hover:opacity-100 transition-opacity" />
                            </div>
                        ))
                    )}
                </div>
            ...
            <div className="h-[44px] bg-studio-panel border-t border-black px-4 flex items-center justify-between shrink-0 shadow-[0_-10px_20px_rgba(0,0,0,0.5)]">
                <div className="flex items-center gap-2 overflow-hidden group/path cursor-pointer">
                    <HardDrive className="w-3.5 h-3.5 text-studio-text-dim group-hover:text-accent-cyan shrink-0" />
                    <span className="text-[10px] font-black text-studio-text-dim truncate uppercase tracking-widest leading-none group-hover:text-studio-text-mid">Macintosh HD › Users › producer › Project</span>
                </div>
                <button className="p-1 hover:bg-white/5 rounded text-studio-text-dim hover:text-white"><Settings className="w-3.5 h-3.5" /></button>
            </div>
