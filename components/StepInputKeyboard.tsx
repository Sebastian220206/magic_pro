            ...
            <div className="bg-[#2c2c2e]/95 backdrop-blur-xl border border-white/10 rounded-xl shadow-[0_30px_90px_rgba(0,0,0,0.8)] overflow-hidden">
                <div className="h-10 flex items-center justify-between px-3 bg-white/5 border-b border-black/30">
                    <div className="flex items-center gap-2">
                        <Piano className="w-3.5 h-3.5 text-sky-300" />
                        <span className="text-[11px] font-black uppercase tracking-widest text-gray-200">Step Editor</span>
                        <span className="text-[10px] text-gray-400">{focusedTrack?.name || 'No Track Selected'}</span>
                    </div>
                    <button onClick={() => toggleStepInput(false)} className="p-1.5 hover:bg-white/10 text-gray-500 hover:text-white rounded-full transition-all">
                ...
                <div className="p-4 space-y-3">
                    <div className="flex flex-wrap items-center gap-2 text-xs text-gray-300">
                        <button onClick={() => setIsSequencerRunning(r => !r)} className={`px-2 py-1 rounded ${isSequencerRunning ? 'bg-emerald-500 text-white' : 'bg-white/5 text-gray-300'}`}>
                        ...
                        <button onClick={() => setMonoMode(m => !m)} className={`px-2 py-1 rounded ${monoMode ? 'bg-violet-500 text-white' : 'bg-white/5 text-gray-300'}`}>Mono {monoMode ? 'On' : 'Off'}</button>
                    ...
                    <div className="flex flex-wrap items-center gap-2 text-xs">
                        ...
                        <select value={gridResolution} onChange={(e) => setGridResolution(e.target.value as any)} className="bg-[#111] border border-white/10 rounded px-2 py-1 text-white">
                        ...
                        <label>Size</label>
                        <select value={sequenceLength} onChange={(e) => setSequenceLength(Number(e.target.value))} className="bg-[#111] border border-white/10 rounded px-2 py-1 text-white">
                        ...
                        <label>Edit</label>
                        <select value={editMode} onChange={(e) => setEditMode(e.target.value as any)} className="bg-[#111] border border-white/10 rounded px-2 py-1 text-white">
                        ...
                        <button onClick={() => setStepTool('pencil')} className={`px-2 py-1 rounded ${stepTool === 'pencil' ? 'bg-sky-500 text-white' : 'bg-white/5 text-gray-300'}`}>Pencil</button>
                        <button onClick={() => setStepTool('eraser')} className={`px-2 py-1 rounded ${stepTool === 'eraser' ? 'bg-rose-500 text-white' : 'bg-white/5 text-gray-300'}`}>Erase</button>
                    </div>
                    ...
                    <div className="space-y-2">
                        ...
                        {rows.map((row, ridx) => (
                            <div key={row.id} className={`rounded border border-white/10 p-2 ${selectedRowId === row.id ? 'ring-2 ring-sky-400' : ''}`}>
                                <div className="flex items-center justify-between gap-2 mb-1 text-xs text-gray-300">
                                    <button onClick={() => setSelectedRowId(row.id)} className="font-semibold text-white">{row.name} ({row.pitch})</button>
                                    <div className="flex items-center gap-1">
                                        <button onClick={() => setRows(prev => prev.map(r => r.id === row.id ? { ...r, muted: !r.muted } : r))} className={`px-1 py-0.5 rounded ${row.muted ? 'bg-rose-500 text-white' : 'bg-white/10 text-gray-300'}`}>M</button>
                                        <button onClick={() => setRows(prev => prev.map(r => r.id === row.id ? { ...r, solo: !r.solo } : r))} className={`px-1 py-0.5 rounded ${row.solo ? 'bg-amber-500 text-black' : 'bg-white/10 text-gray-300'}`}>S</button>
                                    ...
                                    {Array.from({ length: sequenceLength }).map((_, step) => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`h-8 rounded-sm text-[9px] ${isActive ? 'bg-sky-400 text-black' : cellActive ? 'bg-emerald-400 text-black' : 'bg-white/10 text-gray-300'} ${stepTool === 'eraser' ? 'cursor-not-allowed' : 'cursor-pointer'}`}
                                            >
                                                {step + 1}
                                            </button>
                                        )
                    ...
                    <div className="p-2 bg-[#101010] rounded border border-white/10">
                        <div className="text-[10px] uppercase text-gray-400 mb-1">Pitch Selector</div>
                            ...
                            {Array.from({ length: 14 }).map((_, i) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`py-1 text-[10px] rounded ${active ? 'bg-sky-500 text-white' : 'bg-white/10 text-gray-300 hover:bg-white/20'}`}
                                    >
                                        {pitch}
                                    </button>
                                )
                    ...
                    <div className="max-h-44 overflow-y-auto bg-[#090909] border border-white/10 rounded text-xs">
                        {(activeClip?.notes || []).sort((a, b) => (a.startBeat ?? a.start) - (b.startBeat ?? b.start)).map(note => (
                            <div 
                                ...
                                className={`flex items-center justify-between px-2 py-1 hover:bg-white/5 cursor-pointer ${selectedNoteId === note.id ? 'bg-sky-500/30' : ''}`}
                            >
...
function ModifierButton({ active, label, symbol, onClick }: { active: boolean, label: string, symbol: string, onClick: () => void }) {
    return (
        <button 
            onClick={onClick}
            className={`flex flex-col items-center justify-center p-2 rounded-lg border transition-all ${active ? 'bg-sky-500 border-sky-400 text-white shadow-lg' : 'bg-black/20 border-white/5 text-gray-500 hover:text-gray-300'}`}
        >
