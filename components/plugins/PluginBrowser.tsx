...
export function PluginBrowser({ trackId, mode = 'all', onClose }: PluginBrowserProps) {
    ...
    return (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/70 backdrop-blur-sm">
            <div className="w-[860px] max-w-[94vw] h-[620px] max-h-[88vh] bg-studio-panel border border-white/10 rounded-xl shadow-2xl flex flex-col overflow-hidden">
                <header className="flex items-center gap-3 px-4 py-3 border-b border-white/10 shrink-0">
                    <span className="text-[12px] font-bold text-studio-text">
                        {mode === 'instrument' ? 'Instruments' : mode === 'effect' ? 'Audio Effects' : 'Plugins'}
                    </span>
                    <div className="relative flex-1 max-w-xs">
                        <Search className="absolute left-2 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-studio-text-dim" />
                        <input
                            value={query}
                            onChange={e => setQuery(e.target.value)}
                            placeholder="Search…"
                            className="w-full bg-black/40 border border-white/10 rounded pl-7 pr-2 py-1 text-[11px] text-studio-text placeholder:text-studio-text-dim focus:outline-none focus:border-accent-cyan/50"
                        />
                    </div>
                    <button
                        onClick={onClose}
                        className="ml-auto p-1 text-studio-text-dim hover:text-white transition-colors"
                ...
                <div className="flex items-center gap-1 px-4 py-2 border-b border-white/5 overflow-x-auto shrink-0">
                    {categories.map(c => (
                        <button
                            ...
                            className={`px-2.5 py-1 rounded text-[10px] font-semibold whitespace-nowrap transition-colors ${category === c ? 'bg-accent-cyan text-white' : 'text-studio-text-mid hover:text-white hover:bg-white/5'
                                }`}
                        >
                            {c}
                        </button>
                ...
                <div className="flex-1 overflow-y-auto p-4">
                    {state === 'loading' && (
                        <div className="flex items-center justify-center h-full text-studio-text-dim text-[11px] gap-2">
                        ...
                        <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
                            {visible.map(entry => (
                                <div
                                    ...
                                    className="bg-black/30 border border-white/5 rounded-lg p-3 flex flex-col gap-2 hover:border-accent-cyan/40 transition-colors"
                                >
                                    <div className="flex items-start justify-between gap-2">
                                        <div className="min-w-0">
                                            <div className="text-[11px] font-bold text-studio-text truncate">{entry.name}</div>
                                            <div className="text-[10px] text-studio-text-dim truncate">{entry.vendor}</div>
                                        </div>
                                        {entry.isInstrument && (
                                    ...
                                    {entry.description && (
                                        <p className="text-[10px] text-studio-text-dim line-clamp-2">{entry.description}</p>
                                    )}
                                    ...
                                    <button
                                        ...
                                        className="mt-auto px-2 py-1 rounded text-[10px] font-bold bg-white/5 text-studio-text hover:bg-accent-cyan hover:text-white disabled:opacity-40 disabled:hover:bg-white/5 disabled:hover:text-studio-text transition-colors"
                                    >
                            ...
                            {visible.length === 0 && (
                                <div className="col-span-full py-16 text-center text-[11px] text-studio-text-dim">
                ...
                <footer className="px-4 py-2 border-t border-white/5 text-[10px] text-studio-text-dim shrink-0">
