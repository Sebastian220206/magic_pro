...
export function SearchAndSelectDialog() {
    ...
    return (
        <div className="fixed inset-0 z-[1000] flex items-start justify-center pt-24 bg-black/60 backdrop-blur-[4px] animate-in fade-in duration-200">
            <div
                className="w-[480px] bg-[#2c2c2e] border border-white/10 rounded-xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200"
                onKeyDown={(e) => {
            ...
            >
                ...
                <div className="flex items-center px-4 h-12 border-b border-black/40 gap-3">
                    ...
                    <input
                        ...
                        className="flex-1 bg-transparent text-gray-200 text-[13px] font-medium outline-none placeholder:text-gray-500"
                        value={query}
                        ...
                        filtered.map((track, idx) => {
                            ...
                            return (
                                <button
                                    ...
                                    className="w-full flex items-center px-4 h-11 gap-4 hover:bg-sky-500 group transition-all text-left"
                                >
                                    <div className="w-6 text-[11px] font-black text-gray-500 group-hover:text-white/80 tabular-nums text-right">
                                        {trackNum}
                                    </div>
                                    <div
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="text-[13px] font-bold text-gray-200 group-hover:text-white truncate">
                                            {track.name}
                                        </div>
                                        <div className="text-[10px] font-black text-gray-500 group-hover:text-white/60 uppercase tracking-widest">
                                            {track.type}
                                        </div>
                                    </div>
                                    {track.isStack && <Layout className="w-3 h-3 text-gray-500 group-hover:text-white/60 mr-2" />}
                                </button>
                            )
                        })
                    ) : (
                ...
                {/* Footer Tip */}
                <div className="h-9 px-4 flex items-center bg-[#1c1c1e] text-[10px] font-bold text-gray-500 uppercase tracking-widest border-t border-black/40">
