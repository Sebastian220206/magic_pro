  ...
  return (
    <div className="flex h-screen items-center justify-center bg-daw-bg">
      <div className="bg-daw-panel p-10 rounded-xl shadow-2xl border border-daw-border w-96">
        ...
        {googleEnabled && (
          <>
            {/* Same provider as sign-in: Google creates the account on first
                use, so there is no separate "register" flow. */}
            <GoogleSignInButton callbackUrl="/dashboard" action="Sign up" disabled={loading} />
            <AuthDivider />
          </>
        )}
